...
public:
	...
	bool push_chunk_to_window(const bufferChunk_S chunk, EmittedWindow_S* dest); // pass by const ref to avoid copy, non-const because we modify internal state
...
private:
	...
	bool emit_window_payload(EmittedWindow_S* dest); //should it be const? dont think so cuz its modifying data at dest?
	...
	windowStartEpoch_ms_; // for t0
	expectedNextChunkFirstIdx_; // redundancy check
